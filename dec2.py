...
def dec2(input):
    ...
    for row in arr:
        if len(row) > 0:
            splitted = row.split(" ")
            low = int(splitted[0].split('-')[0])
            high = int(splitted[0].split('-')[1])
            rule = splitted[1].replace(":", "")
            password = splitted[2]
            hits = 0
            for char in password:
                if char == rule:
                    hits += 1
            if low <= hits and hits <= high:
                nr_of_correct += 1
    return nr_of_correct
...
def dec2_2(input):
    ...
    for row in arr:
        if len(row) > 0:
            splitted = row.split(" ")
            low = int(splitted[0].split('-')[0])-1
            high = int(splitted[0].split('-')[1])-1
            rule = splitted[1].replace(":", "")
            password = splitted[2]
            hits = 0
            if password[low] == rule:
                hits += 1
            if password[high] == rule:
                hits += 1
